...
public class QuanLySanPhamDAO {
        ...
        try {
            ...
            if (r != null) {
                while (r.next()) {
                    ...
                    dssp.add(new SanPham(masp, loaisp, tensp, dongia, soluong, url,matg,manxb));
                }
            }
    ...
    public Boolean add(SanPham sp) {
        qlspConnection = new ConnectionDB();
        Boolean ok = qlspConnection.sqlUpdate("INSERT INTO `sanpham` (`MaSP`, `MaLSP`, `TenSP`, `DonGia`, `SoLuong`, `HinhAnh`, `MaTG`, `MaNXB`) VALUES ('"
                + sp.getMaSP() + "', '"
                + sp.getMaLSP() + "', '"
                + sp.getTenSP() + "', '"
                + sp.getDonGia() + "', '"
